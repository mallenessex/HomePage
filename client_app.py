...
# Force qtpy to use PyQt6 directly (skip PyQt5/PySide2 fallback chain).
# Must be set BEFORE importing webview or qtpy.
...
def _set_chromium_privacy_flags() -> None:
    """Inject Chromium command-line flags that disable all phone-home behaviour.

    These must be set BEFORE QtWebEngine initialises (i.e. before
    ``webview.start()``).  The ``QTWEBENGINE_CHROMIUM_FLAGS`` env-var is the
    standard way to pass flags to the embedded Chromium inside QtWebEngine.
    """
    privacy_flags = [
        # ── network-level kill switches ──────────────────────────────

        "--disable-background-networking",          # kills Safe-Browsing updates, component updater, etc.
        "--disable-client-side-phishing-detection", # no phishing model downloads
        "--disable-default-apps",
        "--disable-extensions",
        "--disable-component-update",               # no CRLSet / widevine fetches
        "--disable-domain-reliability",             # no domain-reliability pings
        "--disable-sync",                           # no Google-account sync
        "--disable-translate",                      # no Google Translate service
        "--disable-breakpad",                       # no crash-report uploads
        "--no-pings",                               # no <a ping="…"> hyperlink auditing
        "--disable-features="
        ...
        # ── DNS / prediction ─────────────────────────────────────────
        "--dns-prefetch-disable",                   # no speculative DNS
        "--disable-preconnect",                     # no speculative TCP/TLS
        # ── metrics & telemetry ──────────────────────────────────────
        "--metrics-recording-only",                 # collect nothing; send nothing
        "--disable-metrics",
        "--disable-metrics-reporting",
        # ── miscellaneous hardening ──────────────────────────────────
        "--disable-speech-api",                     # no voice-recognition service
        "--disable-remote-fonts",                   # don't fetch Google Fonts, etc.
        "--disable-gpu-shader-disk-cache",          # no disk artefacts for fingerprinting
        "--autoplay-policy=user-gesture-required",  # don't auto-play media
    ]
...
def open_contained_browser(server_url: str) -> None:
    ...
    # ── apply Chromium privacy flags BEFORE the engine starts ───────
    _set_chromium_privacy_flags()
    ...
    start_kwargs = {
        "debug": False,
        "private_mode": False,
        "gui": "qt",  # Use Qt (PyQt6-WebEngine) on all platforms for consistency
    }
...
class ClientApp:
    ...
    def _save_active_to_cfg(self) -> None:
        ...
        # Auto-update label from server name or ID
